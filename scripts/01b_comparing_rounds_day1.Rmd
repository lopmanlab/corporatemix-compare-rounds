...
df_rd3 <- participant_rd3 %>%
  select("participant_id", "participant_age_2", "participant_sex", "race", "hispanic2", 
         "education2", "family_structure", "state_res", "company") %>%
...
# merge round dfs
df_all <- rbind(df_rd1, df_rd2, df_rd3, df_rd4)
...
t1 <- participants %>%
  ...
  bold_labels() %>%
  modify_header(label = "") %>%
  modify_spanning_header(c("stat_1", "stat_2", "stat_3", "stat_4") ~ "**Survey round**") %>%
...
# save table
t1 <- as_tibble(t1)
write.csv(t1, "../output/day1_only/table1_participant_characteristics.csv")
# rm(t1)
...
# save table
t2 <- as_tibble(t2)
write.csv(t2, "..//output/day1_only/table2_contact_characteristics.csv")
# rm(t2a)
...
# all_rounds_contacts <- df_all %>%
#   select("participant_id","round") %>%
#   group_by(participant_id, round) %>%
#   summarise(num_contacts = n())
# 
# ##### CONTINUE FROM HERE
# # for this table, group contacts by each variable separately and join
# 
# df_allb <- left_join(df_all, all_rounds_contacts, 
#                      by=c("participant_id"="participant_id", "round"="round"))
# df_allb$num_contacts <- as.numeric(df_allb$num_contacts)
...
variables <- data.frame(var=c("participant_sex","participant_age", "family_structure",
                              "contact_location", "company","race", "hispanic"), #  "contact_relation",
                        name=c("Sex","Age Group", "Family structure",
                               "Location of contact", "Company","Race","Hispanic")) %>% # "Relationship to contact"
...
write.csv(table_one, "..//output/day1_only/SI_table_day1_only_contacts.csv")
...
box_colors <-  c("#08519c","#3182bd","#6baed6","#bdd7e7") 
...
tiff("../output/day1_onlyfig2_median_all_rounds.tiff", units="in", width=6, height=6, res=300)
p1
dev.off()
