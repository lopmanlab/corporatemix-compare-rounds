...
```{r participant-files, echoFALSE, include=FALSE}
...
participant_rd2 <- participant_rd2 %>%
  select(id, participant_sex, participant_age_2, education2, race, hispanic, 
         family_structure, company) %>% # num_hh
  rename(participant_id = id,
         participant_age = participant_age_2,
         education = education2) %>%
    mutate(round = "Two") %>%
  arrange(company, participant_id, participant_age, participant_sex, education, 
...
participants$racehispanic <- "Hispanic"
participants <- participants %>%
  mutate(racehispanic = case_when(hispanic == "No" & race == "Asian" ~ "Asian, Non-Hispanic",
                                  ...
                                  hispanic == "No" & race == "Mixed" ~ "Other, Non-Hispanic",
                                  TRUE ~ racehispanic),
...
```{r contact-files, echoFALSE, include=FALSE}
# 2. contact files
# names(contact_rd1)
contact_rd1 <- contact_rd1 %>%
  ...
  # mutate(contact_duration = case_when(totaltime == "Less than 5 minutes" ~ "<5 mins",
  #                                     totaltime == "Between 5 to 15 minutes" ~ "5-15 mins",
  #                                     totaltime == "Between 15 minutes to 1 hour" ~ "15 mins-1 hr",
  #                                     totaltime == "Between 1 hour to 4 hours" ~ "1-4 hrs",
  #                                     totaltime == "More than 4 hours" ~ "4+ hrs"),
  #        factor(contact_duration, levels = c("<5 mins", "5-15 mins", "15 mins-1 hr", 
  #                                            "1-4 hrs", "4+ hrs"))) %>%
...
label(contacts$contact_duration) <- "Duration of contact"
...
t2 <- contacts %>%
  select(contact_age, contact_sex, contact_type, contact_location, 
...
```{r, echo=FALSE}
# save data that will be used shared in zenodo

# merge contact & participant data
df_all_merged <- left_join(participants, contacts,
                     by=c("participant_id", "round"))

write_rds(df_all_merged, "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/CorporateMix/2.Contact data/zenodo-datashare/data/raw/df_all_merged.RDS")
...
```{r}
meansd_rd1 <- day_rd1 %>%
  summarise(mean = mean(num_contacts, na.rm = TRUE),
            sd = sd(num_contacts, na.rm = TRUE))
...
# Median contactsat work, home and community by round
```{r}
...
# m_location <- plot_grid(NULL, m1_rd1_work, m1_rd2_work, m1_rd3_work, m1_rd4_work, legend_m1,
#                         NULL, m1_rd1_home, m1_rd2_home, m1_rd3_home, m1_rd4_home, NULL,
#                         NULL, m1_rd1_comm, m1_rd2_comm, m1_rd3_comm, m1_rd4_comm, NULL,
#                         nrow=3, ncol=6, align="hv",
#                         rel_widths=c(.2, 1.2, 1, 1, 1, .2), rel_heights=c(1,1,1.2), scale=0.9,
#                         labels = c('Work','','','', '', '',
#                                    'Home', '', '', '', '','',
#                                    'Community', '', '', '', '',''),
#                         label_size=30, bg='transparent')
