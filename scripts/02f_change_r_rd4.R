...
source("scripts/02b_pre_pandemic.R")
...
df_rd4 <- df_rd4 %>% 
  ...
  mutate(part_id = id,
        cont_id = 1:n(),## unique id for each contact
...
df_rd4_d1 <- df_rd4 %>% filter(contact_day != "Day 2 only") ## Take only day one contacts

rd4 <- survey(
  participants = df_rd4_d1 %>% ungroup() %>%
    select(part_id, age, participant_sex) %>% unique() %>%
    rename("part_age" = "age") %>%
    rename("part_gender" = "participant_sex"),
  
  contacts = df_rd4_d1 %>% select(part_id, cont_id, cnt_gender, 
                                  cnt_age_est_min, cnt_age_est_max,
                                  cnt_home, cnt_work, cnt_school, 
                                  cnt_otherplace)
)

rd4_cm_scaling <- function(...){
  cm_filter(df = rd4, boots = 1000,
            filter = TRUE, 
            symmetric = TRUE, 
            ...
  )
...
rd4_cm <- cm_filter(rd4, boots=1000)
rd4_cm_school <- rd4_cm_scaling(school = 1)
rd4_cm_home <- rd4_cm_scaling(home = 1)
rd4_cm_work <- rd4_cm_scaling(work = 1)
rd4_cm_other <- rd4_cm_scaling(other = 1)
...
eigen_scale_rd4 <- numeric(boots)
rd4_impute <- list()

for (i in 1:1000){
  ## Scaling for each matrix
  res <- scale_factor_R(rd4_cm, pre_cm, i = i)
  eigen_scale_rd4[i] <- res[[1]]
  rd4_impute[[i]] <- res[[2]]
}

home_changeR_rd4 <- length(pre_cm_home)
for (i in 1:1000) {
  home_changeR_rd4[[i]]<- scale_factor1(rd4_cm_home, pre_cm_home,i=i)
}


work_changeR_rd4 <- length(pre_cm_home)
for (i in 1:1000) {
  work_changeR_rd4[[i]]<- scale_factor1(rd4_cm_work, pre_cm_work,i=i)
}

other_changeR_rd4 <- length(pre_cm_home)
for (i in 1:1000) {
  other_changeR_rd4[[i]]<- scale_factor1(rd4_cm_other, pre_cm_other,i=i)
}

rd4_res_list <- list(all_changeR = eigen_scale_rd4,
                     home_changeR = home_changeR_rd4,
                     work_changeR = work_changeR_rd4,
                     other_changeR = other_changeR_rd4)

saveRDS(rd4_res_list, "output/rel_transmission_interim/rd4_res.RDS")
