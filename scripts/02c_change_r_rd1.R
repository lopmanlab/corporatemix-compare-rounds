...
contact_rd1 <- contact_rd1 %>% 
      ...
      mutate(
         ...
         cnt_gender = case_when(
           is.na(contact_gender) ~ NA_character_,
           contact_gender == "Female" ~ "F",
           contact_gender == "Male" ~ "M")
      ) %>% 
...
rd1 <- survey(
  participants = part_rd1 %>%
    select(part_id, age, gender) %>% unique() %>%
    rename("part_age" = "age") %>%
    rename("part_gender" = "gender"),
  
  contacts = contact_rd1_d1 %>% select(part_id, cont_id, cnt_gender, 
...
saveRDS(rd1_res_list, "output/rel_transmission_interim/rd1_res.RDS")
