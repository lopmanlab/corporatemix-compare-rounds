...
df_rd1 <- readRDS("../data/df_day_clean_rd1.RDS") %>%
...
participant_rd1 <- df_rd1 %>%
  ...
  mutate(round = "One")
participant_rd1$participant_age[participant_rd1$participant_age=="40-49"] <- "40-59"
participant_rd1$participant_age[participant_rd1$participant_age=="50-59"] <- "40-59"

# participant_rd1 <- day_rd1 %>%
#   rename(participant_id = part_id,
...
df_rd2 <- participant_rd2 %>%
  select("participant_id", "participant_age_rd1", "participant_sex", "race", "hispanic",
  ...
  rename(participant_age = participant_age_rd1, education = education2) %>%
  # distinct(., participant_id, .keep_all = TRUE) %>%
  mutate(round = "Two")
...
df_rd3 <- participant_rd3 %>%
  select("participant_id", "participant_age_rd1", "participant_sex", "race", "hispanic2", 
         "education2", "family_structure", "state_res", "company") %>%
...
# merge round dfs
df_rounds <- rbind(df_rd1, df_rd2, df_rd3, df_rd4)
```
...
# save table
# t1 <- as_tibble(t1)
# write.csv(t1, "../output/tables/table1_participant_characteristics.csv")
# rm(t1)
...
r2_contacts <- contact_rd2 %>%
  select("participant_id","round") %>%
  filter(round=="Two") %>%
  group_by(participant_id, .drop=FALSE) %>%
  summarise(r2_contacts = n())

r3_contacts <- contact_rd3 %>%
  select("participant_id","round") %>%
  filter(round=="Three") %>%
  group_by(participant_id) %>%
  summarise(r3_contacts = n())

r4_contacts <- contact_rd4 %>%
  select("participant_id","round") %>%
  group_by(participant_id) %>%
  summarise(r4_contacts = n())

r1_mean <- quantile(r1_contacts$r1_contacts, c(0.25, 0.50, 0.75)) #; d0_mean
r2_mean <- quantile(r2_contacts$r2_contacts, c(0.25, 0.50, 0.75)) #; d1_mean
r3_mean <- quantile(r3_contacts$r3_contacts, c(0.25, 0.50, 0.75)) #; d2_mean
r4_mean <- quantile(r4_contacts$r4_contacts, c(0.25, 0.50, 0.75)) #; d3_mean

overall_contacts <- rbind(r1_contacts, r2_contacts, r3_contacts, r4_contacts)

df_roundsb <- left_join(df_rounds, r1_contacts, by=c("participant_id","round"))
df_roundsb <- left_join(df_roundsb, r2_contacts, by="participant_id")
df_roundsb <- left_join(df_roundsb, r3_contacts, by="participant_id")
df_roundsb <- left_join(df_roundsb, r4_contacts, by="participant_id")
...
contact_summary_data <- participants %>%
  right_join(contacts, by=c("participant_id", "round"))
...
variables <- data.frame(var=c("company","participant_sex","participant_age", "family_structure",
                              "race", "hispanic"), # company
                        name=c("Company","Sex","Age group", "Family structure",
                               "Race","Hispanic")) %>% # "Relationship to contact"
...
for (i in 1:nrow(variables)){
  x <- participants[,variables$var[[i]]] # participant[,variables$var[[i]]]
  variables$var[[i]]
...
# Number and proportion of participants in each strata
  ...
  t4 <- contact_summary_data %>%
    ...
    summarise(num_contacts = n()) %>%
    do(data.frame(t(quantile(.$num_contacts, na.rm=TRUE, probs=c(0.25,0.5,0.75))))) # Median and IQR      
  t4$med_contact <- as.character(paste(t4$X50.," (",t4$X25.,"-",t4$X75.,")",sep=""))
  
  # # Median contacts for round 1
  # t1 <- df_all %>%
  #   filter(round =="One") %>%
  #   droplevels(.$company) %>%
  #   group_by(participant_id, variables$var[[i]]) %>%   # group by id and count number of contacts
