...
rd1_sd <- round(sd(rd1_mean_bootstrap$t[, 2]),1)
...
rd2_sd <- round(sd(rd2_mean_bootstrap$t[, 2]),1)
...
rd3_sd <- round(sd(rd3_mean_bootstrap$t[, 2]),1)
...
rd4_sd <- round(sd(rd4_mean_bootstrap$t[, 2]),1)
...
### Proportion of contacts by age

```{r}
## Proportion of contacts by agegroup
prop_contacts <- function(df) {
  df %>%
    # filter(round == "One") %>%
    dplyr::group_by(participant_age, contact_age) %>%
    dplyr::summarize(n=n(), .groups = "drop") %>%
    mutate(prop_contacts = round(100*(n/sum(n)), 1)) %>%
    ggplot(aes(x = participant_age, y = contact_age, fill = prop_contacts)) +
    geom_raster() +
    scale_fill_gradient(low = "blue", high = "red") +
    geom_text(aes(participant_age, contact_age, label = round(prop_contacts, digits=1)), 
              color = "white", size = 8) +
    theme_classic() +
    theme(legend.position = "none") +
    xlab("") + 
    ...
    theme(axis.title.x = element_text(size=26, face="bold", angle=0),
...
prop_contacts_rd1 <- prop_contacts(rd1) + ylab("Contact age")
prop_contacts_rd2 <- prop_contacts(rd2)
prop_contacts_rd3 <- prop_contacts(rd3) + ylab("Contact age") + xlab("Participant age")
prop_contacts_rd4 <- prop_contacts(rd4) + xlab("Participant age")

legend_m_prop <- get_legend(prop_contacts_rd2 +
                          guides(color = guide_legend(nrow = 1)) +
                          theme(legend.position = "right",
                                legend.justification = "left"))

m_prop <- plot_grid(prop_contacts_rd1, prop_contacts_rd2, legend_m_prop,
                    prop_contacts_rd3, prop_contacts_rd4,
          # labels = c("A","B","","C","D"),
          # label_size = 22,
          ncol=3, nrow=2, rel_widths=c(1.2,1.15,.2),
          scale=0.95, align="hv")

# ggsave(m_prop, filename = "../../publication/output/matrix_age_proportions.tiff",
#        height=18, width=20, dpi=75,
#        bg="white")
...
ggsave(m_location, filename = "../../publication/output/matrix_location.tiff",
       height=18, width=20, dpi=75,
       bg="white")
...
# create a divergent color scale theme
diff_color_scale <- scale_fill_gradient2(low="#4d9221", mid="#f7f7f7", high="#c51b7d", midpoint = 0,
                       limits=c(-1, 3), breaks=(c(-1,0,1,3)))

# round 2-1
diff1 <- df_m1_rd1 %>%
  rename(tot_contacts_rd1 = tot_contacts,
         n_rd1 = n,
         avg_cont_rd1 = avg_cont) %>%
  left_join(df_m1_rd2 %>%
              rename(tot_contacts_rd2 = tot_contacts,
                     n_rd2 = n,
                     avg_cont_rd2 = avg_cont), by = c("participant_age", "contact_age")) %>%
  mutate(avg_cont = avg_cont_rd2 - avg_cont_rd1) %>%
  select(participant_age, contact_age, avg_cont_rd2, avg_cont_rd1, avg_cont)
...
# round 3-2
diff2 <- df_m1_rd3 %>%
  rename(tot_contacts_rd3 = tot_contacts,
                     n_rd3 = n,
                     avg_cont_rd3 = avg_cont) %>%
  left_join(df_m1_rd2 %>%
              rename(tot_contacts_rd2 = tot_contacts,
                     n_rd2 = n,
                     avg_cont_rd2 = avg_cont), by = c("participant_age", "contact_age")) %>%
  mutate(avg_cont = avg_cont_rd3 - avg_cont_rd2) %>%
  select(participant_age, contact_age, avg_cont)
...
# round 4-3
diff3 <- df_m1_rd4 %>%
  rename(tot_contacts_rd4 = tot_contacts,
                     n_rd4 = n,
                     avg_cont_rd4 = avg_cont) %>%
  left_join(df_m1_rd3 %>%
              rename(tot_contacts_rd3 = tot_contacts,
                     n_rd3 = n,
                     avg_cont_rd3 = avg_cont), by = c("participant_age", "contact_age")) %>%
  mutate(avg_cont = avg_cont_rd4 - avg_cont_rd3) %>%
  select(participant_age, contact_age, avg_cont)
...
diff4 <- df_m1_rd4 %>%
  ...
  left_join(df_m1_rd1 %>%
              rename(tot_contacts_rd1 = tot_contacts,
                     n_rd1 = n,
                     avg_cont_rd1 = avg_cont), by = c("participant_age", "contact_age")) %>%
  mutate(avg_cont = avg_cont_rd4 - avg_cont_rd1) %>%
  select(participant_age, contact_age, avg_cont)
...
# ggsave(diff_matrix, filename = "../../publication/output/matrix_difference_all.tiff",
#        height=18, width=20, dpi=75,
#        bg="white")
...
```{r}
compute_Q_index <- function(matrix) {
  ...
  q_index <- 1 - (sum(matrix) / total_pairs)
...
m_rd1 <- df_m1_rd1 %>%
  ...
  filter(contact_age!="10-19") %>%  filter(participant_age!="10-19") %>%
  select(avg_cont)
m_rd1 <- matrix(m_rd1$avg_cont,4,4,byrow=F)
  

# Compute the Q-index for the example matrix
rd1_q_value <- compute_Q_index(m_rd1)
cat("RD1 Q-index:", rd1_q_value, "\n")

rd2_q_value <- compute_Q_index(matrix(df_m1_rd2$avg_cont,6,6,byrow=F))
cat("RD2 Q-index:", rd2_q_value, "\n")

rd3_q_value <- compute_Q_index(matrix(df_m1_rd3$avg_cont,6,6,byrow=F))
cat("RD3 Q-index:", rd3_q_value, "\n")

rd4_q_value <- compute_Q_index(matrix(df_m1_rd4$avg_cont,6,6,byrow=F))
cat("RD4 Q-index:", rd4_q_value, "\n")
...
### Weighted matrix
```{r weighted_matrix_dfs, eval=FALSE, warning=FALSE, echo=FALSE, comment = NA, fig.width = 18, fig.height = 14}
...
col_names <- c("20-29","30-39","40-59","60+","0-9","10-19")
row_names <- c("0-9","10-19","20-29","30-39","40-59","60+")
