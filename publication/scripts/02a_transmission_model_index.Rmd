...
source("../../publication/scripts/00b_model_functions.R")
...
df <- data.frame(value = c(Reduce(c,rd1_res_list), 
                           ## Reduce list of vectors into one vector, combined into column in data frame
...
ggplot(df, aes(value, fill = round)) + geom_density() +
  ...
  xlab("Relative transmissibility due to contact changes compared to pre-pandemic")+
  ylab("Density") 
