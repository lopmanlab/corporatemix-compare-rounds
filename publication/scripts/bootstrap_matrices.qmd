...
m_rd1 <- contact_matrix(df_rd1, 
                        age.limits = c(0,10,20,30,40,50,99), ## Specify age bands for the matrix
...
# the output from here has age group 40+ and omits 40-59 and 60+. that's why the matrices below are the way they are.

##################
# here, i tried to update the code to reflect the updates in the socialmixr vignette (https://cran.r-project.org/web/packages/socialmixr/vignettes/socialmixr.html#bootstrapping)
# i get similar output. mean values are not the same, but very close.

# m_rd1 <- replicate(n=1000,
#                    contact_matrix(df_rd1,
#                         # does this only work for square matrices? currently, age group 40-59 and 60+ are aggregated as one group of 40+.
#                         age.limits = c(0,10,20,30,40,100), ## Specify age bands for the matrix
#                         sample.participants = TRUE))
# mr <- Reduce("+", lapply(m_rd1["matrix", ], function(x) x / ncol(m_rd1)))
####################
